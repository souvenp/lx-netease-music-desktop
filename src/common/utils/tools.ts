...
export const toNewMusicInfo = (oldMusicInfo: any): LX.Music.MusicInfo => {
  ...
  if (oldMusicInfo.source == 'local') {
    ...
    if (oldMusicInfo.originCoverType != null || oldMusicInfo.meta?.originCoverType != null) {
      meta.originCoverType = oldMusicInfo.originCoverType ?? oldMusicInfo.meta.originCoverType
    }
    if (oldMusicInfo.mv != null || oldMusicInfo.meta?.mv != null) meta.mv = oldMusicInfo.mv ?? oldMusicInfo.meta.mv
    if (Array.isArray(oldMusicInfo.artists)) {
      meta.artists = oldMusicInfo.artists.map((artist: any) => ({
        id: artist.id,
        name: artist.name,
      })).filter((artist: any) => artist.id != null && artist.name)
    }
    if (oldMusicInfo.fee != null) meta.fee = oldMusicInfo.fee
