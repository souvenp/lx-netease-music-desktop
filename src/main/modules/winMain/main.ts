...
import { NETEASE_MUSIC_REQUEST_HEADERS } from '@common/constants'
...
export const registerNeteaseCookieMusicUrl = (url: string) => {
  const normalizedUrl = normalizeUrl(url)
  neteaseCookieMusicUrls.add(normalizedUrl)
  setTimeout(() => {
    neteaseCookieMusicUrls.delete(normalizedUrl)
  }, 15 * 60_000)
}

const setNeteaseMusicRequestHeaders = (ses: Electron.Session) => {
  ses.webRequest.onBeforeSendHeaders({
    ...
    if (!neteaseCookieMusicUrls.has(normalizeUrl(details.url))) {
...
export const createWindow = () => {
  ...
  setNeteaseMusicRequestHeaders(ses)
