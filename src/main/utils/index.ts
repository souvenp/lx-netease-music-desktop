...
const isObjectValueEqual = (originValue: Record<string, unknown>, targetValue: Record<string, unknown>): boolean => {
  const keys = new Set([...Object.keys(originValue), ...Object.keys(targetValue)])
  for (const key of keys) {
    if (originValue[key] !== targetValue[key]) return false
  }
  return true
}
const resolveSettingValue = <K extends keyof LX.AppSetting>(
  originValue: LX.AppSetting[K],
  targetValue: unknown,
): { shouldUpdate: boolean, value?: LX.AppSetting[K] } => {
  if (checkPrimitiveType(targetValue)) {
    return targetValue == originValue
      ? { shouldUpdate: false }
      : { shouldUpdate: true, value: targetValue as LX.AppSetting[K] }
  }

  if (isPlainObject(targetValue) && isPlainObject(originValue)) {
    const originObject: Record<string, unknown> = originValue
    const targetObject: Record<string, unknown> = targetValue
    const mergedValue = {
      ...originObject,
      ...targetObject,
    } as unknown as LX.AppSetting[K]

    return isObjectValueEqual(originObject, mergedValue as Record<string, unknown>)
      ? { shouldUpdate: false }
      : { shouldUpdate: true, value: mergedValue }
  }

  return { shouldUpdate: false }
}
const setSettingValue = <K extends keyof LX.AppSetting>(setting: Partial<LX.AppSetting>, key: K, value: LX.AppSetting[K]) => {
  setting[key] = value
}
const setFullSettingValue = <K extends keyof LX.AppSetting>(setting: LX.AppSetting, key: K, value: LX.AppSetting[K]) => {
  setting[key] = value
}
  ...
  if (targetSetting) {
    ...
    if (originSettingKeys.length > targetSettingKeys.length) {
      for (const key of targetSettingKeys as Array<keyof LX.AppSetting>) {
        ...
        if (originSettingCopy[key] === undefined) continue
        const result = resolveSettingValue(originSettingCopy[key], targetValue)
        if (!result.shouldUpdate) continue
        updatedSettingKeys.push(key)
        setSettingValue(updatedSetting, key, result.value!)
        setFullSettingValue(originSettingCopy, key, result.value!)
      }
    } else {
...
const applyInitSetting = (setting: LX.AppSetting) => {
  ...
  setting['download.enable'] = true
  setting['list.actionButtonsVisible'] = true
  setting['odc.isAutoClearSearchInput'] = false
  setting['odc.isAutoClearSearchList'] = false
}
