...
const getDownloadHeaders = (downloadInfo: LX.Download.ListItem) => {
  return downloadInfo.metadata.isNeteaseCookieUrl ? {} : undefined
}

const shouldSkipDefaultUserAgent = (downloadInfo: LX.Download.ListItem) => {
  return !!downloadInfo.metadata.isNeteaseCookieUrl
}
...
const createTask = async(downloadInfo: LX.Download.ListItem, savePath: string, skipExistFile: boolean, proxy?: { host: string, port: number }) => {
  ...
  const downloadOptions: DownloadOptions = {
    ...
    method: 'get',
    headers: getDownloadHeaders(downloadInfo),
    skipDefaultUserAgent: shouldSkipDefaultUserAgent(downloadInfo),
    proxy,
    onCompleted() {
...
export const updateUrl = (id: string, url: string) => {
  ...
  const dl = dls.get(id)
  if (!dl) return
  dl.refreshUrl(url, getDownloadHeaders(task), shouldSkipDefaultUserAgent(task))
  dl.start().catch(err => {
    sendAction(id, {
      action: 'error',
      data: {
...
export const startTask = async(downloadInfo: LX.Download.ListItem, savePath: string, skipExistFile: boolean, callback: (action: LX.Download.DownloadTaskActions) => void, proxy?: { host: string, port: number }) => {
  ...
  if (dl) {
    ...
    if (downloadInfo.metadata.url) dl.refreshUrl(downloadInfo.metadata.url, getDownloadHeaders(downloadInfo), shouldSkipDefaultUserAgent(downloadInfo))
