...
const toSerializableMusicInfos = <T extends LX.Music.MusicInfo>(musicInfos: T[]): T[] => {
  return musicInfos.map(info => JSON.parse(JSON.stringify(toRaw(info))))
}
...
export const addListMusics = async(id: string, musicInfos: LX.Music.MusicInfo[], addMusicLocationType?: LX.AddMusicLocationType) => {
  return addListMusicsAction({
    id,
    musicInfos: toSerializableMusicInfos(musicInfos),
