...
import { wyScrobble } from '@renderer/store/user/action'
...
export default () => {
  ...
  const handleSetPlayInfo = () => {
    ...
    wyScrobbleState = currentMusicInfo && 'source' in currentMusicInfo && currentMusicInfo.source == 'wy'
      ? { id: currentMusicInfo.meta.songId, reported: false }
      : null
  }

  const getWyScrobbleSourceId = () => {
    const id = tempListMeta.id?.startsWith('wy__') ? tempListMeta.id.replace(/^wy__/, '') : playMusicInfo.listId
    return id || 0
  }

  const tryWyScrobble = (time: number) => {
    if (!wyScrobbleState || wyScrobbleState.reported || !playProgress.maxPlayTime) return
    const threshold = Math.min(240, Math.max(30, playProgress.maxPlayTime / 2))
    if (time < threshold) return
    wyScrobbleState.reported = true
    void wyScrobble(wyScrobbleState.id, getWyScrobbleSourceId(), time).catch(err => {
      console.log(err)
    })
  }
