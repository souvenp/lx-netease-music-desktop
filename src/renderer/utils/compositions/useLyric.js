...
export default ({ isPlay, lyric, playProgress, isShowLyricProgressSetting, offset }) => {
  ...
  const handleLyricClick = event => {
    const dom = event.target.closest?.('.line-content')
    if (!dom || !dom_lyric.value?.contains(dom) || dom.time == null) return
    let targetTime = Math.max(dom.time - lyric.offset - lyric.tempOffset, 0) / 1000
    if (targetTime > playProgress.maxPlayTime) targetTime = playProgress.maxPlayTime
    isStopScroll.value = false
