...
export default {
  ...
  async getListDetail(rawId, page, tryNum = 0) { // 获取歌曲列表内的音乐
    ...
    const requestCookie = cookie || appSetting['common.wy_cookie'].trim() || this.cookie
    ...
    const requestObj_listDetail = httpFetch('https://music.163.com/weapi/v3/playlist/detail', {
      ...
      form: weapi({
        ...
        n: this.limit_song,
        s: 8,
        csrf_token: (requestCookie.match(/_csrf=([^;]+)/) || [])[1] || '',
      }),
    })
    const { statusCode, body } = await requestObj_listDetail.promise
    if (statusCode !== 200 || body.code !== this.successCode) {
      if (body.code === 401) throw new Error(body.message || 'Netease playlist needs login')
      return this.getListDetail(rawId, page, ++tryNum)
    }
    if (!body.playlist?.trackIds?.length) {
      return {
        list: [],
        page,
        ...
        source: 'wy',
        info: {
          play_count: formatPlayCount(body.playlist?.playCount || 0),
