  ...
  <div :class="$style.container">
    <div :class="$style.songListHeader">
      ...
      <div :class="$style.songListHeaderRight">
        ...
        >
          {{ listDetailInfo.source == 'wy' ? '收藏到本地' : $t('list__collect') }}
        </base-btn>
        <base-btn
          v-if="listDetailInfo.source == 'wy'"
          :class="$style.headerRightBtn"
          :disabled="!!listDetailInfo.noItemLabel"
          @click="handleWySubPlaylist"
        >
          {{ isWySubscribed ? '取消收藏' : '收藏到网易云' }}
        </base-btn>
...
import { toggleWySubPlaylist } from '@renderer/store/user/action'
import { wySubscribedPlaylists } from '@renderer/store/user/state'
...
export default {
  ...
  setup() {
    ...
    const isWySubscribed = computed(() => wySubscribedPlaylists.some(item => String(item.id) == listDetailInfo.id))
    const tempListId = computed(() => `${listDetailInfo.source}__${listDetailInfo.id}`)
    ...
    const restoreScroll = () => {
      ...
      void nextTick(() => {
        listRef.value?.scrollToIndex(Number(index))
      })
    }
    ...
    const handleWySubPlaylist = () => {
      void toggleWySubPlaylist({
    ...
    watch([source, id, page, refresh], async([_source, _id, _page, _refresh]) => {
      ...
      // page.value = _page ?? 1
      void getListData(_source, _id, _page, _refresh).then(() => {
