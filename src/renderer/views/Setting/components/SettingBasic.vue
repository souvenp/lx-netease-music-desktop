...
dd
  div
    ...
    .gap-top(:class="$style.wyCookie")
      label(for="setting_wy_cookie") 网易云音乐 Cookie
      base-input(
        id="setting_wy_cookie"
        :model-value="appSetting['common.wy_cookie']"
        placeholder="在此处粘贴网易云 Cookie"
        :trim="false"
        @change="updateSetting({'common.wy_cookie': $event})"
      )
  h3#basic_nav_menu {{ $t('setting__basic_nav_menu') }}
  div
    .gap-top(v-for="item in navMenuList" :key="item.id")
      base-checkbox(
        :id="`setting_nav_menu_${item.id}`"
        :model-value="item.enabled"
        :label="item.label"
        :disabled="item.alwaysVisible"
        @update:model-value="updateNavStatus(item.id, $event)"
      )
    .p.gap-top
...
dd
  h3#basic_background 背景设置
  div
    .gap-top
      base-checkbox(id="setting_theme_background_enable" :model-value="appSetting['theme.background.enable']" label="启用自定义背景图片" @update:model-value="updateSetting({'theme.background.enable': $event})")
    .gap-top
      base-checkbox(id="setting_theme_background_dynamic" :model-value="appSetting['theme.background.dynamic']" label="动态背景（使用歌曲封面）" @update:model-value="updateSetting({'theme.background.dynamic': $event})")
    .p.gap-top
      base-btn.btn(min @click="handleSelectBackground") 选择自定义图片
      base-btn.btn(min :disabled="!appSetting['theme.background.image']" @click="clearBackground") 清空
      span.auto-hidden(:class="$style.bgPath") {{ appSetting['theme.background.image'] || '未选择' }}
    .p.gap-top(:class="$style.sliderRow")
      span 模糊 {{ appSetting['theme.background.blur'] }}px
      base-slider-bar(:class="$style.slider" :value="appSetting['theme.background.blur']" :min="0" :max="80" @change="updateSetting({'theme.background.blur': $event})")
    .p.gap-top(:class="$style.sliderRow")
      span 透明度 {{ appSetting['theme.background.opacity'] }}%
      base-slider-bar(:class="$style.slider" :value="appSetting['theme.background.opacity']" :min="0" :max="100" @change="updateSetting({'theme.background.opacity': $event})")
  ...
  setup() {
    ...
    const handleSelectBackground = () => {
      void showSelectDialog({
        ...
        updateSetting({
          'theme.background.image': `file:///${encodePath(filePath)}`,
          'theme.background.enable': true,
        })
      })
    }
    const clearBackground = () => {
      updateSetting({ 'theme.background.image': '' })
    }
    ...
    const navMenuList = computed(() => {
      ...
      return NAV_MENUS.map(item => ({
        id: item.id,
        label: item.i18nKey ? t(item.i18nKey) : item.label,
        alwaysVisible: Boolean(item.alwaysVisible),
        enabled: (item.alwaysVisible ?? false) || (navStatus[item.id] ?? true),
      }))
    })
    const updateNavStatus = (id, enabled) => {
      updateSetting({
        'common.navStatus': {
          ...(appSetting['common.navStatus'] ?? {}),
          [id]: enabled,
        },
      })
    }
