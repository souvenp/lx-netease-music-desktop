      ...
      div.left(:class="$style.left")
        ...
        div(:class="$style.info")
          ...
          div.description(:class="['scroll', $style.description]")
            ...
            p
              span {{ $t('player__music_singer') }}
              template(v-if="neteaseArtists.length")
                template(v-for="(artist, index) in neteaseArtists" :key="artist.id || artist.name")
                  span(v-if="index" :class="$style.artistSep") 、
                  button(type="button" :class="$style.infoLink" @click="handleOpenNeteaseArtist(artist)") {{ artist.name }}
              span(v-else) {{ musicInfo.singer }}
            p(v-if="musicInfo.album")
              span {{ $t('player__music_album') }}
              button(v-if="neteaseAlbum" type="button" :class="$style.infoLink" @click="handleOpenNeteaseAlbum") {{ musicInfo.album }}
...
export default {
  ...
  setup() {
    ...
    const handleOpenNeteaseArtist = (artist) => {
      openNeteaseArtist(artist)
      hide()
    }
    const handleOpenNeteaseAlbum = () => {
      openNeteaseAlbum()
      hide()
    }
    ...
    return {
      ...
      hide,
      handleContextMenu,
      handleOpenNeteaseArtist,
      handleOpenNeteaseAlbum,
      hideComment,
      handleAfterEnter,
      handleAfterLeave,
      visibled,
