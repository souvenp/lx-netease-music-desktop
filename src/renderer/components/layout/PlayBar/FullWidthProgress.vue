<template>
  <div :class="$style.player">
    ...
    <div :class="$style.infoContent">
      <div :class="$style.title" :aria-label="title + $t('copy_tip')" @click="handleCopy(title)" @dblclick.stop="handleToMusicLocation">
        <span>{{ musicInfo.name }}</span>
        <template v-if="musicInfo.singer">
          <span> - </span>
          <template v-if="neteaseArtists.length">
            <template v-for="(artist, index) in neteaseArtists" :key="artist.id || artist.name"><span v-if="index">、</span><button type="button" :class="$style.titleLink" @click.stop="openNeteaseArtist(artist)">{{ artist.name }}</button></template>
          </template>
          <span v-else>{{ musicInfo.singer }}</span>
        </template>
      </div>
...
import useNeteaseMusicLinks from '../useNeteaseMusicLinks'
import useToMusicLocation from './useToMusicLocation'
...
export default {
  ...
  setup() {
    ...
    const {
      neteaseArtists,
      openNeteaseArtist,
    } = useNeteaseMusicLinks()
    const { handleToMusicLocation } = useToMusicLocation()
